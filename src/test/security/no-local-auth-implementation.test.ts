...
describe("no local auth implementation guard", () => {
  ...
  it("requires @tgoliveira/secure-auth 0.12.0 and resolves that release", () => {
    const packageJson = JSON.parse(readSource("package.json")) as {
      dependencies: Record<string, string>;
    };
    expect(packageJson.dependencies["@tgoliveira/secure-auth"]).toBe("^0.12.0");

    const lockfile = readSource("package-lock.json");
    expect(lockfile).toContain('"@tgoliveira/secure-auth": "^0.12.0"');
    expect(lockfile).toContain("secure-auth-0.12.0.tgz");
  });
