...
| CSP | Nonce-based, production `script-src 'self' 'nonce' 'strict-dynamic' 'wasm-unsafe-eval'`; `object-src 'none'`, `frame-ancestors 'none'`, `base-uri 'self'`, `form-action 'self'`, `worker-src 'self' blob:`; no third-party **scripts**. `connect-src` is `'self'` plus the (self-hostable) on-device voice model host(s) and, only when enabled, the exact validated HTTPS portable broker origin — see `src/lib/security/content-security-policy.ts` |
